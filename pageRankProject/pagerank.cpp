...
public: 
	dictionaryHash(); 
	dictionaryHash(int size); 
	int Hash(string key);
	int returnIndex(string key); 
	void addItem(string name);
	void addEdge(string name, int element); 
	int itemCountInIndex(int index); 
	void search(string item, int& count, string arr[]); 
	void retrieve(int hashIndex, string rec); 
	void remove(int hashIndex, string rec);  
}; 
...
int main(int argc, char *argv[])
{
	...
	{
		string webpages = ""; 
		ifstream readObj(current->data + ".txt"); 
